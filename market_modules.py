...
def initialize():
    ...
    global agents, all_agents, exposed_agents, susceptible_agents, infected_agents, asymptomatic_agents, symptomatic_agents, asymptomatic1_agents, asymptomatic2_agents ,symptomatic1_agents, symptomatic2_agents,  time1, quarantined, quarantined1, quarantined2,quarantined3
    ...
    global exposed_plot, susceptible_plot, infected_plot, symptomatic_plot, asymptomatic_plot,symptomatic1_plot,symptomatic2_plot, susceptible_plot1,susceptible_plot2 ,asymptomatic1_plot,asymptomatic2_plot
    global prob_exposed, incubation_period, prob_infection, recovery_susceptible,asymptomatic_recovery, risk_life, social_radius, eff_quarantined, quarantined_recovery , hospital_capacity, essentials_move
    global asympto_in, sympto_in, asympto_out, sympto_out
    ...
    symptomatic2_plot =[0]
    asymptomatic1_plot =[num_citizens_infected - math.ceil(num_citizens_infected / 2)]
    asymptomatic2_plot =[0]
    
    susceptible_plot1=[0]
    susceptible_plot2=[(num_agents - num_citizens_infected)]
...
def observe():
    ...
    asymptomatic1_agents = gpd.GeoDataFrame([ag.geometry for ag in agents if ag.status == 'infected' and ag.specifics == 'asymptomatic' and ag.quarantined == 'no'],geometry='geometry',columns=['geometry'], crs=edges_proj.crs)
    ...
    if len(asymptomatic1_agents) > 0:
        asymptomatic1_agents.plot(ax=ax, marker='o', alpha=1,markersize=10, color=my_colors[2], zorder=5)
    ...
    for colorx, places in [('yellow','marketplace'), ('magenta','hospital')]:
        ax.scatter([], [], c=colorx, s=100, label=places, alpha=1, edgecolors='none',marker='s')
    leg1=ax.legend(frameon=True,title='',labelspacing=1,loc="upper left",fontsize=14)
...
def update_main():
    ...
    if  random.random() > ((focal_citizen.trust_official  +  focal_citizen_network) / 2) : 
        if len(focal_citizen.route) == 0 : # if route list is empty
            ...
            focal_citizen_neighbors = [j for j in agents if (j.quarantined == 'no') and ((j.geometry).within(focal_citizen.geometry.buffer(social_radius))==True) and (j != focal_citizen)]
...
def update_one_unit_time():
    ...
    for ag in agents :
        ...
        elif all([(ag.status == 'infected'),(ag.specifics == 'asymptomatic')]) :
    ...
    counter_xx = 0 # track number  quarantined per day
    counter_xx1=0 # track number of symptomatic quarantined per day
    counter_xx2=0 # track number of asymptomatic quarantined per day
    for ag in agents :
        if all([(ag.status == 'infected'),(ag.specifics == 'symptomatic'),(ag.quarantined == 'no'),(random.random() < eff_quarantined)]):
            ...
            counter_xx += 1
            counter_xx1 += 1
        elif all([(ag.status == 'infected'),(ag.specifics == 'asymptomatic'),(ag.quarantined == 'no'),(random.random() < eff_quarantined)]):
            ag.quarantined = 'yes'
            counter_xx += 1
            counter_xx2 += 1
    quarantined1.append(counter_xx) # comes into quarantine box
    sympto_in.append(counter_xx1) 
    asympto_in.append(counter_xx2) 
      

    counter_yy = 0 # track recovered
    counter_yy1 = 0 # sympto recovered
    counter_yy2 = 0 # asympto recovered
        ...
        if all([(ag.status == 'infected'),(ag.specifics == 'symptomatic'),(ag.quarantined == 'yes')]):
            ...
            if (ag.time_quarantined > quarantined_recovery) :
                if random.random() < (1-(sum([1 for j in agents if j.quarantined == 'yes']) / float(hospital_capacity * num_agents))) : 
                    ...
                    counter_yy += 1
                    counter_yy1 += 1
        elif all([(ag.status == 'infected'),(ag.specifics == 'asymptomatic'),(ag.quarantined == 'yes'),(ag.time_asymptomatic > asymptomatic_recovery)]):
            ag.quarantined = 'no'
            ag.status = 'susceptible'
            ag.specifics = 'recovered'
            counter_yy += 1
    ...
    sympto_out.append(counter_yy1) 
    asympto_out.append(counter_yy2) 
    ...
    susceptible_plot.append(sum([1 for j in agents if j.status == 'susceptible'])) # total number of susceptibles
    susceptible_plot1.append(sum([1 for j in agents if j.status == 'susceptible'and j.specifics == 'recovered'])) # number of susceptibles immune
    susceptible_plot2.append(sum([1 for j in agents if j.status == 'susceptible'and j.specifics == 'null'])) # number of susceptibles 
    ...
    asymptomatic1_plot.append(sum([1 for j in agents if j.status == 'infected' and j.specifics == 'asymptomatic' and j.quarantined == 'no' ])) 
    asymptomatic2_plot.append(sum([1 for j in agents if j.status == 'infected' and j.specifics == 'asymptomatic' and j.quarantined == 'yes' ])) 
    ...
    header = ['susceptible_all','exposed', 'infected_all','asymptomatic_all','asympto_not_quarantined','asympto_quarantined','symptomatic_all','sympto_not_quarantined','sympto_quarantined', 'in_per_day','out_per_day','remaining_per_day','sympto_in','asympto_in','sympto_out','asympto_out', 'suscept_immune','suscept_NoTimmune']
    main_data = [susceptible_plot, exposed_plot, infected_plot,asymptomatic_plot, asymptomatic1_plot, asymptomatic2_plot ,symptomatic_plot,symptomatic1_plot,symptomatic2_plot,quarantined1, quarantined2, quarantined3,sympto_in,asympto_in,sympto_out,asympto_out,susceptible_plot1,susceptible_plot2]
